...
use hal::{pac, prelude::*, stm32, serial::Serial, rcc, gpio::gpioa, gpio::AF7};
...
impl SerialStruct {
    fn new() -> Self {
        ...
        let pac::Peripherals { RCC: destruct_RCC, DMA1: destruct_DMA1, FLASH: destruct_FLASH, GPIOA: destruct_gpioa, .. } = dp;
        let mut rcc = destruct_RCC.constrain();
        ...
        let mut delay = hal::delay::Delay::new(core_periphs.SYST, clocks);
        ...
        let serial = Serial::usart1(dp.USART1, pins, 115200.bps(), clocks, &mut apb2);
...
fn main() -> ! {
    ...
    let mut initial_buf = singleton!(: [u8; 9] = *b"hello DMA").unwrap();
